...
const initDatabase = (creds) => {
  const config = {
    maxAttempt: 10,
    plugins: [{ retry: { retryStatusCodes: [429] } }]
  };
  // APIキーがあり、パスワードがない場合は IAMをプラグインにセットする。
  if (creds.apikey && !creds.password) {
    config.account = creds.username;
    config.plugins.push({ iamauth: { iamApiKey: creds.apikey } });
  } else {
    // URL・ユーザー名、パスワードで認証する。
    config.url = creds.url;
  }
  const cloudant = new Cloudant(config);
...
class NlcStub {
  ...
  getClassifier (params, callback) {
    ...
    this.nlc.get(params.classifierId, (error, value) => {
      if (error) {
        console.log('error:', error);
        checkUnauthorized(error, callback);
  ...
  createClassifier (params, callback) {
    ...
    try {
      ...
      const metadata = JSON.parse(params.trainingMetadata);
      name = metadata.name;
      language = metadata.language;
    } catch (e) {
      throw new Error('Missing trainingMetadata');
    }
    ...
    this.listClassifiers({}, (error, value) => {
      if (error) {
        ...
        if (typeof params.trainingData === 'string') {
          insert(this.nlc, name, language, params.trainingData, callback);
        } else {
          let csv = '';
          params.trainingData.on('data', (data) => {
            csv += data;
          });

          params.trainingData.on('end', () => {
